...
class UserSerializer(serializers.ModelSerializer):
    class Meta:
        model = Users
        read_only_fields = ["id"]
        exclude = ['password', 'is_superuser', 'first_name', 'last_name', 'avatar']
        extra_kwargs = {
            'post': {'required': False},
        }
...
class UserGroup(APIView):
    ...
    user_view_get_resp = {'200': ReturnUserSerializer,
                          '400': "Bad Request"}
    ...
    @swagger_auto_schema(
        request_body=openapi.Schema(
            ...
            properties={
                ...
                "groupid": openapi.Schema(type=openapi.TYPE_INTEGER, description='组别Id', ),
            },
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        userinfo = Users.objects.filter(id=userid)
        return_data = UserSerializer(instance=userinfo, many=True)
        return SuccessResponse(data=return_data.data, message='新增成功')
